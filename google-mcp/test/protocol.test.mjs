...
test('advertises exactly the read-only tools, all marked read-only', async () => {
  await withServer(async (server) => {
    ...
    assert.deepEqual(tools.map((tool) => tool.name).sort(), ['list_events', 'search_messages'])
...
test('search_messages returns trimmed metadata with parsed addresses and bulk flags', async () => {
  await withServer(async (server) => {
    ...
    const [person, bulk] = result.items
    assert.equal(person.id, 'm0')
    assert.equal(person.thread_id, 't0')
    assert.equal(person.from_address, 'jane0@example.com')
    assert.deepEqual(person.to_addresses, ['owner@example.org', 'other@example.net'])
    assert.equal(person.snippet, "Can we meet & talk? It's urgent")
...
test('list_events walks calendars in order, drops cancelled events and resources', async () => {
  await withServer(async (server) => {
    ...
    assert.deepEqual(
      ...
      ['owner@example.org', 'pat@example.com'],
    )
